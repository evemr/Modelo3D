...
class Light extends GLJPanel implements GLEventListener, KeyListener {
    ...
    // Dibuja la mesa
    setSomeGrayMaterial(gl, GL.GL_FRONT);
    gl.glPushMatrix();
    {
        gl.glTranslatef(0.0f, -1.5f, 0.0f);
        gl.glScalef(2.0f, 0.2f, 1.0f);  // Ajusta las dimensiones de la mesa
        glut.glutSolidCube(1.0f);
    }
    gl.glPopMatrix();

    // Dibuja la dona
    setSomeRedMaterial(gl, GL.GL_FRONT);
    gl.glPushMatrix();
    {
        gl.glTranslatef(0.0f, 0.5f, 0.0f);
        glut.glutSolidTorus(0.2f, 0.5f, 20, 20);  // Ajusta las dimensiones de la dona
    }
    gl.glPopMatrix();
}
